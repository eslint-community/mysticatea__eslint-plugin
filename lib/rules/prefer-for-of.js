...
function isCalledRecursively(context, node) {
    return (
        node.id != null &&
        getDeclaredVariables(context, node)[0].references.length > 0
    )
}
 ...
 * @param {ASTNode} startNode The node to check.
...
function isIndexVarOnlyUsedToGetArrayElements(context, node) {
    ...
    const indexVar = getDeclaredVariables(context, node.init)[0]
...
function isLengthVarOnlyUsedToTest(context, node) {
    ...
    const lengthVar = getDeclaredVariables(
        context,
 ...
 *
 * @param {Scope.Scope} startScope - The rule context to get variables.
 * @param {string} name - The variable name to get.
 * @returns {escope.Variable|null} The found variable.
 */
function getVariableByName(startScope, name) {
...
function getContextVariable(context, contextNode) {
    ...
    const scope = getScope(context, contextNode).upper
